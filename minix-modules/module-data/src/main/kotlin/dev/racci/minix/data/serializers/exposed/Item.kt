 ...
 */
public fun Entity<*>.itemStack(column: Column<ExposedBlob>): ItemStackExposedDelegate = ItemStackExposedDelegate(column)
public fun Entity<*>.nullableItemStack(column: Column<ExposedBlob?>): ItemStackExposedDelegateNullable = ItemStackExposedDelegateNullable(column)
...
private fun toItemStack(byteArray: ByteArray): ItemStack {
    return ByteArrayInputStream(byteArray).use {
        BukkitObjectInputStream(it).use { item ->
            item.readObject() as ItemStack
        }
    }
}
