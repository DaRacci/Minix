...
class MutableMultiMap<K, V> : MultiMap<K, V> {
    ...
    override val isEmpty get() = internalMap.isEmpty()

    override val keys get() = internalMap.keys.toImmutableSet()

    override val entries get() = internalMap.entries.toImmutableSet()

    override val values get() = internalMap.values.toImmutableSet()
    ...
    operator fun plusAssign(value: Pair<K, V>) = this.put(value.first, value.second)

    /** Removes the specified value with the specified key in this multimap. */
    operator fun minusAssign(value: Pair<K, V>) { this.remove(value.first, value.second) }

    /** Removes this key (and its corresponding value) from this multimap. */
    operator fun minusAssign(key: K) { this.remove(key) }
     ...
     */
    fun remove(key: Any?) = internalMap.remove(key)?.toImmutableSet()
     ...
     */
    fun clear(onRemove: (K, ImmutableSet<V>) -> Unit) {
