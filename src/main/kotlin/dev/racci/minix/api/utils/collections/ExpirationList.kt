...
fun <E> MinixPlugin.expirationListOf(): ExpirationList<E> = ExpirationListImpl(this)

fun <E> WithPlugin<*>.expirationListOf(): ExpirationList<E> = plugin.expirationListOf()
...
class ExpirationListImpl<E>(private val plugin: MinixPlugin) : ExpirationList<E> {
    ...
    override fun missingTime(element: E): Int? = getNodeByElement(element)
        ?.let { it.expireTime - (System.currentTimeMillis() - it.startTime / 1000) }
        ?.toInt()

    override operator fun contains(element: E) = indexOf(element) > -1

    override operator fun get(index: Int): E? = getNode(index)?.element
