...
object LazyUtils : UtilObject by UtilObject {
    ...
    suspend inline fun <R> ifInitialised(
        value: KProperty0<R>,
        action: R.() -> Unit
    ) = if (isInitialised(value)) value.accessGet().action() else Unit

    @JvmName("ifInitialized1")
    suspend inline fun <T : Any, R> ifInitialised(
        value: KProperty1<T, R>,
        obj: T,
        action: R.() -> Unit
    ) = if (isInitialised(value, obj)) value.accessGet(obj).action() else Unit
}
